...
const Register = () => {
    ...
    const onSubmit = async (values) => {
        ...
        const response = axios.post(`${process.env.API_URL}/api/v1/auth/signup`, data).catch((err) => {
            if (err && err.response)
    ...
    const CreateAccSchema = Yup.object().shape({
        fullName: Yup.string().min(3, "name must be atleast 3 chracters long").required("Please Enter your name"),
